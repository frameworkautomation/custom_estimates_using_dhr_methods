...
def parse_args():
    ...
    ap.add_argument("--go-home",   action="store_true",    help="after the grab, move all joints (incl. j7) to 0")
    ...
    return {
        ...
        "go_home":   a.go_home   or CONFIG_GO_HOME,
...
def main():
    ...
    try:
        ...
        print(f"[RESULT] Position error    : {pos_err:.4f} mm")
        print(f"[RESULT] Z-axis angle err  : {angle_deg:.4f} deg")
        ...
        if not converged:
            print(f"[FAIL] Could not reach '{cfg['target']}' within tolerances "
                  ...
                  f"Best achievable from this seed: {pos_err:.3f} mm / {angle_deg:.3f} deg.")
