...
7. **Automatic collision check for rear boxes/bin** *(challenging but feasible)* — scripted collision detection for rear box and bin positions
...
9. **Add cone movement to DHR's code** *(hard)* — integrate our end effector into their state machine (end effector up and down), use their pipeline for collision checking, understand their IO setup
...
10. **Understand PLC interaction with end effector** *(hard)* — how the PLC communicates with and controls the end effector, and how to program it
